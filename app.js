...
//create a new user and send to databse
app.post('/sign_up', function(req,res){
	...
	var sql = "INSERT INTO User_tbl (user_id, username, eMail, password, name, age) VALUES (Null,'"+username+"','"+email+"','"+password+"','"+name+"','"+age+"');";
...
app.post('/sign_in', function(req,res){
	...
	if(username && password){
		connection.query("SELECT * FROM User_tbl WHERE username = ? AND password = ?", [username, password], function(error, results, fields){
			if(results.length > 0){
